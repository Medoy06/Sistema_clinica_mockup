...
// Role → capability flags. Mirrors the backend permissions map (cosmetic;
// backend still enforces). Used to decide which data to fetch AND which
// widgets to render, so a role never triggers a call it would be 403'd on.
const capabilities = (role?: string) => ({
  clinical: ['admin', 'doctor', 'recepcionista', 'enfermera'].includes(role ?? ''),
  inventory: ['admin', 'farmaceutico', 'recepcionista', 'bodega'].includes(role ?? ''),
});
...
export const DashboardPage = ({ onNavigate }: DashboardPageProps) => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        // Clinical data — only fetched for clinical roles.
        if (caps.clinical) {
          const [appts, notifs, patients, doctors] = await Promise.all([
        ...
        // Inventory data — only fetched for inventory roles.
        if (caps.inventory) {
  ...
  return (
    <div className="p-6">
      ...
      {/* Quick stats — each card gated by capability */}
      ...
      {/* Two column previews — each gated */}
