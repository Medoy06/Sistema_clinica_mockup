...
export const InventoryPage = () => {
  const { items, lowStockItems, categories, loading, error, createItem, deleteItem, refetch } = useInventory();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setShowForm(false);
      setForm({ name: '', description: '', category_id: '', unit: '', quantity: 0, min_quantity: 0, unit_price: 0, location: '' });
    } catch (err) {
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">{items.length} artículos registrados</p>
        </div>
        <button
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map(item => (
                <tr key={item.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`font-medium ${item.quantity <= item.min_quantity ? 'text-red-600' : 'text-slate-800'}`}>
                      {item.quantity}
                    </span>
                  </td>
